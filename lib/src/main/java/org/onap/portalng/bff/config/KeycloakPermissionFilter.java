                ...
                new Expiry<String, List<CachedPermission>>() {
                  ...
                  public long expireAfterCreate(
                    ...
                    return value.isEmpty()
                        ...
                        : Duration.ofMinutes(5).toNanos(); // safe fallback, overridden by policy.put()
                  }
  ...
  private Mono<List<CachedPermission>> fetchPermissions(
    ...
    return webClient
        ...
        .doOnNext(
            permissions -> {
              if (jti != null) {
                permissionCache.policy().expireVariably().ifPresent(policy -> {
                  policy.put(jti, permissions, ttl.toNanos());
                });
              }
            });
  }
    ...
    try {
      return Mono.just(
          objectMapper.readValue(
              response, new TypeReference<List<CachedPermission>>() {}));
    } catch (Exception e) {
