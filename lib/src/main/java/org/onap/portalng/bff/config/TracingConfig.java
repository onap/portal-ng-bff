...
/**
 * Keeps the requests the platform polls - Kubernetes probes and the Prometheus scrape - out of the
 * exported traces. Dropping the observation also drops their {@code http_server_requests} series.
 */
@Configuration
public class TracingConfig {
  ...
  ObservationPredicate untracedPathsPredicate(
      @Value("${bff.tracing.untraced-paths-enabled:true}") boolean enabled) {
    final AntPathMatcher pathMatcher = new AntPathMatcher();
    return (name, context) -> {
      if (!enabled || !(context instanceof ServerRequestObservationContext serverContext)) {
        return true;
      }
      final String path = serverContext.getCarrier().getPath().pathWithinApplication().value();
